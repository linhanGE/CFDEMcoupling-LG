...
#include "diffCentre2DVoidFraction.H"
...
defineTypeNameAndDebug(diffCentre2DVoidFraction, 0);
...
diffCentre2DVoidFraction::diffCentre2DVoidFraction
(
...
:
    ...
    realDepth_(readScalar(propsDict_.lookup("realDepth"))),
    virtualDepth_(readScalar(propsDict_.lookup("virtualDepth")))
{
...
void diffCentre2DVoidFraction::setvoidFraction(double** const& mask,double**& voidfractions,double**& particleWeights,double**& particleVolumes,double**& particleV) const
{
    reAllocArrays();
...
void diffCentre2DVoidFraction::buildLabelHashSet
(
    const scalar radius,
